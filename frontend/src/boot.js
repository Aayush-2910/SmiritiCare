  ...
  const pending = (async () => {
    ...
    const dependencies = await Promise.all(
      imports.map(async (match) => [
        match[1],
        await loadModule(new URL(match[1], url).href),
      ]),
    );
    for (const [path, dependency] of dependencies) {
      code = code
        .replaceAll(`'${path}'`, `'${dependency}'`)
        .replaceAll(`"${path}"`, `"${dependency}"`);
    }
...
start().catch((error) => {
  ...
  document.getElementById("root").innerHTML =
    '<main class="initial-loading"><span>🌿</span><h2>A little pause.</h2><p>SmritiCare could not load. Please check your connection and try again.</p><button onclick="location.reload()">Try again</button></main>';
});
